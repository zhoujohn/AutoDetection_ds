 ...
 * prt BASE
 */
public abstract class PtrBase<T extends View> extends ViewGroup {
    ...
    private IHeaderLayout mHeaderStateCallback;
    private IPtrHandler mPtrHandler;
    // working parameters
    ...
    public void setmDisableWhenVMove(boolean mDisableWhenVMove) {
        this.mDisableWhenVMove = mDisableWhenVMove;
    }

    private boolean mDisableWhenVMove = false;
    private int mFlag = 0x00;

    // disable when detect moving horizontally
    private boolean mPreventForHorizontal = false;
    private boolean mPreventForV= false;
    ...
    public PtrBase(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.PtrBase, 0, 0);
        if (arr != null) {
            mHeaderId = arr.getResourceId(R.styleable.PtrBase_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.PtrBase_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.PtrBase_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.PtrBase_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.PtrBase_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.PtrBase_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.PtrBase_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.PtrBase_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
        ...
        setRefreshView(context, attrs);

        addPullHeader();

    }

    private void setRefreshView(Context context, AttributeSet attrs) {
        mRefreshView = getRefreshView(context, attrs);
        if (mRefreshView != null) {
            addView(mRefreshView, new RecyclerView.LayoutParams(RecyclerView.LayoutParams.MATCH_PARENT, RecyclerView.LayoutParams.MATCH_PARENT));
        }
    }

    private void addPullHeader() {
        setHeaderCallback(getPtrHeader());
        setHeaderView((View) mHeaderStateCallback);
    }
    ...
    protected void onFinishInflate() {
        ...
        for (int x = 0; x < childCount; x++) {
           // LogUtil.log("getChildAt(x):" + getChildAt(x));
        }
        if (mHeaderView != null) {
        ...
        if (mRefreshView != null) {
            ...
            if (isDebug()) {
                ...
                PtrCLog.d(LOG_TAG, "onMeasure, currentPos: %s, lastPos: %s, top: %s",
                        mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mRefreshView.getTop());
            }
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        if (!isEnabled() || mRefreshView == null || mHeaderView == null) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                LogUtil.log("offsetX:" + Math.abs(offsetX) + " offsetY:" + Math.abs(offsetY));
                LogUtil.log("mDisableWhenHorizontalMove:" + mDisableWhenHorizontalMove + "  mPreventForHorizontal:" + mPreventForHorizontal);


                if (mDisableWhenHorizontalMove && !mPreventForHorizontal && (Math.abs(offsetX) > Math.abs(offsetY) * 2)) {
                    // if (mPtrIndicator.isInStartPosition()) {
                    LogUtil.log("offset in X");
                    mPreventForHorizontal = true;
                    return dispatchTouchEventSupper(e);
                    //  }
                }
                if (mDisableWhenVMove && !mPreventForV && (Math.abs(offsetY) > Math.abs(offsetX) * 2)) {
                    // if (mPtrIndicator.isInStartPosition()) {
                    LogUtil.log("offset in Y");
                    mPreventForV = true;
                    return dispatchTouchEventSupper(e);
                    //  }
                }
                ...
                if (DEBUG) {
                    boolean canMoveDown = mPtrHandler != null && mPtrHandler.checkCanDoRefresh(this, mRefreshView, mHeaderView);
                ...
                LogUtil.log("1111111111111111111");
                // disable move when header not reach top
                if (moveDown && mPtrHandler != null && !mPtrHandler.checkCanDoRefresh(this, mRefreshView, mHeaderView)) {
                    return dispatchTouchEventSupper(e);
                }
                LogUtil.log("222222222222222222222");
                if ((moveUp && canMoveUp) || moveDown) {
                    LogUtil.log("333333333333333333333333");
                    movePos(offsetY);
                    return true;
                }
        }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            mHeaderStateCallback.onHeaderRefreshPrepare(this);
            if (DEBUG) {
                PtrCLog.i(LOG_TAG, "IHeaderLayout: onHeaderRefreshPrepare, mFlag %s", mFlag);
            }
        }
        ...
        if (DEBUG) {
            PtrCLog.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
                    change, mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mRefreshView.getTop(), mHeaderHeight);
        }
        ...
        if (mHeaderStateCallback != null) {
            mHeaderStateCallback.onHeaderPositionChange(this, isUnderTouch, mStatus, mPtrIndicator);
        }
    ...
    private void performRefresh() {
        ...
        if (mHeaderStateCallback != null) {
            mHeaderStateCallback.onHeaderRefresh(this);
            if (DEBUG) {
                PtrCLog.i(LOG_TAG, "IHeaderLayout: onHeaderRefresh");
            }
        }
        if (mPtrHandler != null) {
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator.isInStartPosition()) {
            if (mHeaderStateCallback != null) {
                mHeaderStateCallback.onHeaderReset(this);
                if (DEBUG) {
                    PtrCLog.i(LOG_TAG, "IHeaderLayout: onHeaderReset");
                }
            }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mHeaderStateCallback != null) {
            if (DEBUG) {
                PtrCLog.i(LOG_TAG, "IHeaderLayout: onHeaderRefreshComplete");
            }
            mHeaderStateCallback.onHeaderRefreshComplete(this);
        }
    ...
    public void autoRefresh(boolean atOnce, int duration) {
        ...
        if (OffsetToRefresh == 0) {
            OffsetToRefresh = DensityUtils.dip2px(104);
            duration = duration * 2;
        }
    ...
    public void setPtrHandler(IPtrHandler ptrHandler) {
     ...
     * 添加头部回调
     *
     * @param headerStateCallback
     */
    public void setHeaderCallback(IHeaderLayout headerStateCallback) {
        mHeaderStateCallback = headerStateCallback;
    }
     ...
     * 添加下拉刷新header,自定义样式
     */
    public abstract IHeaderLayout getPtrHeader();

    public abstract T getRefreshView(Context context, AttributeSet attrs);
